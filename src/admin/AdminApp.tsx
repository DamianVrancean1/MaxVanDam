...
import {
  ...
  Search, ChevronRight, RefreshCw, Bell, Download,
  ...
  return (
    <aside className="dm-sidebar">
      ...
      <nav className="dm-nav">
        ...
        <NavLink to="/admin/dashboard" className={({ isActive }) => `dm-nav-link${isActive ? ' active' : ''}`}>
        ...
        <NavLink to="/admin/products" end className={({ isActive }) => `dm-nav-link${isActive ? ' active' : ''}`}>
          <Package className="dm-nav-icon" />
          Produse
        </NavLink>

        <NavLink to="/admin/products/new" className={({ isActive }) => `dm-nav-link${isActive ? ' active' : ''}`}>
        ...
        <NavLink to="/admin/users" className={({ isActive }) => `dm-nav-link${isActive ? ' active' : ''}`}>
          <Users className="dm-nav-icon" />
          Utilizatori
        </NavLink>

        <NavLink to="/admin/subscriptions" className={({ isActive }) => `dm-nav-link${isActive ? ' active' : ''}`}>
          <CreditCard className="dm-nav-icon" />
          Abonamente
        </NavLink>

        <NavLink to="/admin/comenzi" className={({ isActive }) => `dm-nav-link${isActive ? ' active' : ''}`}>
          <ShoppingCart className="dm-nav-icon" />
          Comenzi
        </NavLink>

        <NavLink to="/admin/settings" className={({ isActive }) => `dm-nav-link${isActive ? ' active' : ''}`}>
      ...
      <div className="dm-sidebar-footer">
        ...
        <div className="dm-user-row">
          <div className="dm-user-avatar">{user ? initials(user.username) : 'AD'}</div>
          ...
          <button
            title="Deconectare"
            onClick={onLogout}
            style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--t-tertiary)', padding: 2, lineHeight: 1, transition: 'color 0.15s' }}
  ...
  return (
    <header className="dm-topbar">
      ...
      <div className="dm-search">
        ...
        <div className="dm-search-kbd"><kbd>⌘</kbd><kbd>K</kbd></div>
      </div>

      <div className="dm-topbar-actions">
        <button className="dm-icon-btn" title="Reîncarcă" onClick={() => window.location.reload()}><RefreshCw size={15} /></button>
...
/* ── shell ── */
const AdminShell = ({ children, onLogout }: { children: React.ReactNode; onLogout: () => void }) => {
